...
//    }
    ...
    open func updateLabelDisplay() {
        ...
        if let stepValue = stepValue {
            let a = round(rangeSlider!.lowerValue / stepValue )
            let b = round(rangeSlider!.upperValue / stepValue )
      
            self.lowerLabel?.text = dataArray[Int(a)]
            self.upperLabel?.text = dataArray[Int(b)]
        }
